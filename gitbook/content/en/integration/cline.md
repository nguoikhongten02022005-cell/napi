...
Integrate Napi with Cline VSCode extension to route your AI requests through Napi's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- Napi running locally or cloud endpoint configured
...
Set the base URL to your Napi endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your Napi endpoint
...
3. The key should start with `sk-napi-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your Napi configuration
...
4. Cline will use Napi to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using Napi
...
3. Get AI-powered explanations through Napi

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use Napi to understand context and make changes
...
1. Verify Napi is running: `curl http://localhost:20000/health`
2. Check that the base URL is correct and includes `/v1`
3. Ensure no firewall is blocking port 20000
...
2. Verify your Napi instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check Napi logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use Napi cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://napi.com`
2. Make sure you have configured your API key in the Napi cloud dashboard
...
## Integration with Napi Features

### Model Routing

Napi automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, Napi automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through Napi dashboard:
- Total requests
- Token usage
- Cost per model
