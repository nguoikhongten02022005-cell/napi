...
Napi is compatible with any tool that supports the OpenAI API format. This guide covers generic integration patterns for various tools and custom applications.

## Overview

Napi provides an OpenAI-compatible API endpoint that works with:
...
Any OpenAI-compatible tool can connect to Napi using these settings:
...
**Problem:** Cannot connect to Napi
```bash
...
- Check port 20000 is not blocked
...
- Check available models: `curl http://localhost:20000/v1/models`
...
- Check network connection to Napi
