...
Tích hợp Napi với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Napi.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Napi đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint Napi:
...
3. Nhận giải thích AI qua Napi

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Napi để hiểu context và thực hiện thay đổi
...
1. Xác minh Napi đang chạy: `curl http://localhost:20000/health`
2. Kiểm tra base URL đúng và bao gồm `/v1`
3. Đảm bảo không firewall nào chặn port 20000
...
2. Xác minh Napi instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Napi để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng Napi cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://napi.com`
2. Đảm bảo bạn đã cấu hình API key trong Napi cloud dashboard
...
## Tích hợp với Tính năng Napi

### Định tuyến Model

Napi tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Napi tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Napi dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
