...
Tích hợp Napi với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của Napi.
...
Napi cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong Napi dashboard
...
1. Xác minh tên model khớp với cấu hình Napi
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong Napi dashboard
...
Đảm bảo bạn đã cấu hình API key trong Napi cloud dashboard.
