...
NapiをCline VSCode拡張機能と統合し、AIリクエストをNapiのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- Visual Studio Codeがインストール済み
- VSCodeマーケットプレイスからCline拡張機能がインストール済み
- Napiがローカルで動作中、またはクラウドエンドポイントが設定済み
- NapiダッシュボードからのAPIキー
...
**手順:**
1. **Base URL** フィールドにNapiエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
2. ClineはNapiを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
1. Napiが動作中か確認: `curl http://localhost:20000/health`
2. Base URLが正しく、`/v1` を含むことを確認
3. ファイアウォールがポート20000をブロックしていないか確認
4. VSCodeを再起動してみる
...
localhostの代わりにNapiクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `https://napi.com`
2. NapiクラウドダッシュボードでAPIキーが設定されていることを確認
...
Napiは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、Napiは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

Napiダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
