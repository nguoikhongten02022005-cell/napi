...
開発と個人利用のため、ローカルマシンでNapiを実行。

---

## 📦 インストール

npm経由でNapiをグローバルインストール:
...
- **APIエンドポイント**: `http://localhost:20000/v1`
- **データディレクトリ**: `~/.napi`
...
APIポート(20000)とダッシュボードポート(3000)はアプリケーションで設定されています。変更するにはソースコードを修正するか、サポートされている場合は環境変数を使用してください。

---

## 🛑 サーバーの停止

Napiが実行されているターミナルで `Ctrl+C` を押します。
