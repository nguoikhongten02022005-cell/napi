...
Despliega Napi en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.napi` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/napi
sudo chown $USER:$USER /var/lib/napi
```
...
# Iniciar Napi con PM2
pm2 start npm --name napi -- start
...
Crea `/etc/nginx/sites-available/napi`:
...
# Si NO usas proxy reverso, permite los puertos de Napi
...
tar -czf napi-backup-$(date +%Y%m%d).tar.gz /var/lib/napi

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/napi-$(date +\%Y\%m\%d).tar.gz /var/lib/napi
```
...
# Conexiones de red
netstat -tulpn | grep -E '3000|20000'
```
...
```bash
# Verificar si Napi está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/napi
chmod 755 /var/lib/napi
```
