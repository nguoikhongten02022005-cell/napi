...
// Check if settings has Napi customModels
const hasNapiConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Napi"));
};
...
// POST - Update Napi customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Napi configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Napi"));
...
// DELETE - Remove Napi customModels only (keep other settings)
