...
// Check if settings has Napi config
const hasNapiConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["napi"];
};

// Read per-agent models.json and return current model id (without "napi/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["napi"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasNapi: hasNapiConfig(settings),
...
// POST - Update Napi settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `napi/${model}`;

    // Remove all old napi/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("napi/"))
    ...
    // Add fresh napi models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`napi/${m}`] = {};
    });

    // Remove old napi model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("napi/")) {
    ...
    // Update models.providers.napi with all models
    settings.models.providers["napi"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `napi/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Napi settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Napi from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["napi"];
    ...
    // Remove napi models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("napi/"));
    ...
    // Reset agents.defaults.model.primary if it uses napi
    if (settings.agents?.defaults?.model?.primary?.startsWith("napi/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Napi settings removed successfully",
    });
  } catch (error) {
