...
const hasNapiConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["napi"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["napi"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasNapi: hasNapiConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("napi/") ? config.model.replace(/^napi\//, "") : null,
...
// POST - Apply Napi as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_napi";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing napi provider entry and its models
    const existingProvider = config.provider["napi"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["napi"] = existingProvider;
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `napi/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("napi/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Napi provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["napi"]?.models) {
      delete config.provider["napi"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["napi"].models).length === 0) {
        delete config.provider["napi"];
        if (config.model?.startsWith("napi/")) delete config.model;
      } else if (config.model === `napi/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["napi"].models);
        config.model = `napi/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire napi provider
      if (config.provider) delete config.provider["napi"];
      if (config.model?.startsWith("napi/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("napi/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Napi settings removed from OpenCode",
    });
  } catch (error) {
