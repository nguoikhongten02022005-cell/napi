...
const hasNapiConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Napi");
};

const getNapiEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Napi") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getNapiEntry(config);
...
// POST - Apply Napi config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Napi entry or append
    const idx = config.findIndex((e) => e.name === "Napi");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Napi entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Napi");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Napi removed from Copilot config",
    });
  } catch (error) {
