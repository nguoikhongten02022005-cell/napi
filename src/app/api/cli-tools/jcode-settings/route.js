...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-napi.env");
};
...
const hasNapiConfig = (config) => {
  ...
  if (providers["napi"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:20000")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasNapi = hasNapiConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile napi",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "napi configuration removed from jcode",
    });
  } catch (error) {
