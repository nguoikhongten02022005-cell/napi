...
// Collect PIDs of all napi-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from napi install dir or wrapper cli.js
        const isAppProcess = lower.includes("napi") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("napi") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "napi");
  }
  return path.join(os.homedir(), ".napi");
}
...
// Resolve npx/napi binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx napi` — works regardless of global bin path changes after npm i -g
