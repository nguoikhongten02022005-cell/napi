...
function collapseTextBlocks(parts) {
  if (!Array.isArray(parts) || parts.length === 0) return null;
  if (!parts.every((part) => part && part.type === "text")) return null;
...
function convertClaudeMessage(msg) {
  ...
  if (Array.isArray(msg.content)) {
    ...
    const collapsedParts = collapseTextBlocks(parts);

    // If has tool results, return array of tool messages
    if (toolResults.length > 0) {
      if (parts.length > 0) {
        const textContent = collapsedParts ?? (parts.length === 1 && parts[0].type === "text"
          ? parts[0].text
          : parts);
    ...
    if (parts.length > 0) {
      return {
        role,
        content: collapsedParts ?? (parts.length === 1 && parts[0].type === "text" ? parts[0].text : parts)
      };
    }
