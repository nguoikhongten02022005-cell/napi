...
function isAnthropicNonStreamingRequest(targetUrl, options) {
  ...
  if (hostname !== "api.anthropic.com") return false;
  const accept = getHeaderValue(options?.headers, "accept");
  return !accept.toLowerCase().includes("text/event-stream");
}

async function fetchViaGotScraping(targetUrl, options) {
  ...
  return new Response(response.rawBody ?? response.body ?? null, {
    status: response.statusCode ?? response.status ?? 200,
    statusText: response.statusMessage ?? response.statusText ?? "OK",
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  if (isAnthropicNonStreamingRequest(targetUrl, options)) {
    try {
      return await fetchViaGotScraping(targetUrl, options);
    } catch (error) {
