...
async function buildCodexHeader() {
  ...
  const { installed, hasNapi, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasNapi, settings } = result.data;
  ...
  // Extract Napi custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Napi-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Napi provider config
  const provider = settings?.models?.providers?.["napi"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("napi/") ? primary.replace("napi/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasNapi, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Napi\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
