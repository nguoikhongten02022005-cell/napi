...
function getDbPath() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "napi", "db.json")
    : path.join(os.homedir(), ".napi", "db.json");
}
  ...
  await showMenuWithBack({
    ...
    headerContent: async (data) => {
      ...
      if (tunnel.enabled && tunnel.publicUrl) {
        ...
        lines.push(`  Endpoint: http://localhost:20000/v1`);
 ...
 * and user is locked out of dashboard. CLI bypasses auth via x-napi-cli-token.
 */
