...
console.log("📦 Building Napi CLI package with Next.js...\n");
...
if (fs.existsSync(path.join(standaloneRootToUse, "server.js"))) {
  ...
  const nestedDirs = fs.existsSync(standaloneRootToUse)
    ? fs.readdirSync(standaloneRootToUse, { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .map((entry) => path.join(standaloneRootToUse, entry.name))
      .filter((dir) => fs.existsSync(path.join(dir, "server.js")))
    : [];
  standaloneApp = nestedDirs[0] || path.join(standaloneRootToUse, "app");
}
if (!fs.existsSync(standaloneApp)) {
  console.error("❌ Next.js standalone build not found under .next/standalone");
  console.error("Expected .next/standalone/server.js, or a nested dir containing server.js");
  process.exit(1);
}
...
// Strip better-sqlite3 (native) — it lives in ~/.napi/runtime to avoid
...
if (fs.existsSync(betterDir)) {
  ...
  console.log("✅ Stripped better-sqlite3 (lives in ~/.napi/runtime)");
}
console.log("");
