...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.napi/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/napi/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[napi][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[napi][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[napi][runtime] chmod tray bin failed: ${e.message}`);
  }
}
...
function ensureTrayRuntime({ silent = false } = {}) {
  if (process.platform === "linux" && (isAndroidLike() || (!process.env.DISPLAY && !process.env.WAYLAND_DISPLAY))) {
