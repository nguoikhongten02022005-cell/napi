...
function getDataDir() {
  ...
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || os.homedir(), "napi")
...
function ensureRuntimeDir() {
  ...
  if (!fs.existsSync(pkgPath)) {
    fs.writeFileSync(pkgPath, JSON.stringify({
      ...
      version: "1.0.0",
      private: true,
      description: "User-writable runtime deps for napi (better-sqlite3 native binary)",
    }, null, 2));
  }
  return dir;
}
...
function ensureSqliteRuntime({ silent = false } = {}) {
  if (isAndroidLike() || isHeadlessLinux()) {
